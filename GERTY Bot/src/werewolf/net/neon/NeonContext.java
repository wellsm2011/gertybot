...
public class NeonContext extends ForumContext {
	...
	public static final String RULES_URL = DOMAIN
			+ "viewtopic.php?f=179&t=15538";
	...
	private NeonContext() {
		super(Integer.parseInt(Utils.getProperty("neonId")), Utils
				.getProperty("neonUsername"),
				Utils.getProperty("neonPassword"), DOMAIN,
				NeonGameRecord.INSTANCE, SIGNUPS, PM_MANAGER, POLL_INTERVAL,
				RULES_URL);
	}

	public NeonContext(int userId, String username, String password) {
		super(userId, username, password, DOMAIN, NeonGameRecord.INSTANCE,
				SIGNUPS, setupAltPmManager(), POLL_INTERVAL, RULES_URL);
	}

	@Override
	public List<ForumThread> getGameThreads() throws IOException {
		LinkedList<ForumThread> output = new LinkedList<ForumThread>();
		List<ForumThread> threads = new NeonBoard(WEREWOLF_BOARD).getThreads();
		for (ForumThread thread : threads) {
			if (thread.isStickied() || PARSE_ALL_THREADS)
				output.add(thread);
		}
		return output;
	}
	...
	public boolean checkLogin(final HtmlPage page) throws IOException {
		...
		List<?> loginList = page
				.getByXPath("//table[@id='maintable']/tbody/tr[2]/td/a[text()='Login']");

		// Don't try to log in unless not currently logged in.
		if (loginList.size() > 0) {
			System.out.println("Logging into Neon server as " + LOGIN_USER
					+ "...");
			HtmlAnchor a = (HtmlAnchor) loginList.get(0);
			String url = a.getAttributesMap().get("href").getValue()
					.replaceAll("^\\.", DOMAIN);
			System.out.println(url);
			pageRequestLock();
			HtmlPage page2 = (HtmlPage) CLIENT.getPage(url);
			...
			return false;
		} else if (sidLink.size() > 1) { // The logout button always has an sid.
	...
	@Override
	public void makePost(HtmlPage postPage, final String body,
		...
		for (HtmlForm form : forms) {
			...
			HtmlElement message = form.getElementsByAttribute("textarea",
					"name", "message").get(0);
			message.click();
			message.type(body);
			...
			if (postPage.getUrl().getRef() == null) {
				System.out
						.println("NeonContext: Unable to post - trying again...\nURL: "
								+ initialPage.getUrl().toString());
				try {
				...
				makePost((HtmlPage) initialPage.refresh(), body, subject);
			}
			return;
		}
	...
	public List<PrivateMessage> getPMs(int page) throws IOException {
		...
		HtmlPage initialPage = getPage(DOMAIN + "ucp.php?i=pm&start=" + page
				* 25);
		String baseXPath = "//div[@id='pagecontent']/form/table[last()]/tbody/tr";
		Iterator<?> repliedTo = initialPage.getByXPath(
				baseXPath + "/td[1]/img[last()]").iterator();
		Iterator<?> newMessage = initialPage.getByXPath(
				baseXPath + "/td[1]/img[last()]").iterator();
		Iterator<?> messageLink = initialPage.getByXPath(
				baseXPath + "/td[2]/span/a[last()]").iterator();
		Iterator<?> senderLink = initialPage.getByXPath(
				baseXPath + "/td[3]/p/a[last()]").iterator();
		Iterator<?> timestamp = initialPage.getByXPath(baseXPath + "/td[1]")
				.iterator();
	...
	@Override
	public void makePm(HtmlPage pmPage, String[] to, String[] bcc,
		...
		pmPage = setPmRecipients(setPmRecipients(pmPage, bcc, true), to, false);
		...
		for (HtmlForm form : pmPage.getForms()) {
			...
			message.type(body);
			pagePostLock();
			pmPage = ((HtmlElement) pmPage
					.getFirstByXPath("//input[@name='post' and @class='btnmain']"))
					.click();
	...
	private HtmlPage setPmRecipients(HtmlPage pmPage, String[] users,
			boolean bcc) throws IOException {
		if (users.length == 0)
			return pmPage;
		HtmlElement usernames = (HtmlElement) pmPage
				.getFirstByXPath("//textarea[@name='username_list']");
		usernames.click();
		for (String user : users)
			usernames.type(user + "\n");
		return ((HtmlElement) pmPage.getFirstByXPath("//input[@name='add_"
				+ (bcc ? "bcc" : "to") + "']")).click();
	}

	@Override
	public String getBoardUrl(String boardId) {
		return DOMAIN + "viewforum.php?f=" + boardId;
	}

	@Override
	public String getThreadUrl(String boardId, String threadId) {
		return DOMAIN + "viewtopic.php?t=" + threadId;
	}

	@Override
	public String getThreadUrl(String boardId, String threadId, int start) {
		return DOMAIN + "viewtopic.php?t=" + threadId + "&start="
				+ ((start - 1) * 15);
	}

	@Override
	public String getThreadReplyUrl(String boardId, String threadId) {
		return DOMAIN + "posting.php?mode=reply&f=" + boardId + "&t="
				+ threadId;
	}

	@Override
	public String getPostDeleteUrl(String boardId, String threadId, int postId) {
		return DOMAIN + "posting.php?mode=delete&f=" + boardId + "&p=" + postId;
	}

	@Override
	public String getPostEditUrl(String boardId, String threadId, int postId) {
		return DOMAIN + "posting.php?mode=edit&f=" + boardId + "&p=" + postId;
	}

	@Override
	public String getComposeMessageUrl(int messageId) {
		return DOMAIN + "ucp.php?i=pm&mode=compose&action=reply&p=" + messageId;
	}

	@Override
	public String getComposeMessageUrl() {
		return DOMAIN + "ucp.php?i=pm&mode=compose";
	}

	@Override
	public String getReadMessageUrl(int id) {
		return DOMAIN + "ucp.php?i=pm&mode=view&p=" + id;
	}

	@Override
	public String getDeleteMessageUrl(int id) {
		return DOMAIN + "ucp.php?i=pm&mode=compose&action=delete&p=" + id;
	}

	@Override
	public String getPostUrl(String threadId, int start, int postId) {
		return DOMAIN + "viewtopic.php?p=" + postId + "#p" + postId;
	}

	@Override
	public String getUserProfileUrl(int userId) {
		return DOMAIN + "memberlist.php?mode=viewprofile&u=" + userId;
	}
	...
	public String spoiler(String title, String text) {
		return header(title) + "[spoiler]" + text + "[/spoiler]";
	}
	...
	public String getMessageBoxUrl(String folder, int page) {
		return DOMAIN + "ucp.php?i=pm&folder=" + folder + "&start="
				+ (page * 25);
	}
	...
	private static PmManager setupAltPmManager() {
		HashMap<String, PmFolder> folders = new HashMap<String, PmFolder>();
		return new PmManager(folders);
	}
}
