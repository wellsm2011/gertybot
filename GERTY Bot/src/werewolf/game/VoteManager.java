...
public class VoteManager implements Serializable
{
	...
	private Hashtable<User, LinkedList<Vote>> getSortedVotes()
	{
		...
		while (iter.hasNext())
		{
			...
			if (!applied.contains(vote.getVoter()) || this.multipleVotes)
			{
				...
				{
					VoteManager.LOGGER.warning("Vote target removed from game: " + vote + ". Ignoring vote.");
					continue;
				}
	...
	public void placeVote(Vote vote)
	{
		...
		if (!validVoter)
		{
			VoteManager.LOGGER.warning("Unknown voter: " + vote + ". Ignoring vote.");
			return;
		}
		...
		if (!validTarget)
		{
			VoteManager.LOGGER.warning("Unknown vote target: " + vote + ". Ignoring vote.");
			return;
		}
		...
		while (iter.hasNext())
		{
			...
			if (voteCheck.getVoter().equals(vote.getVoter()))
			{
				...
				{
					VoteManager.LOGGER.fine("Duplicate vote found, ignoring: " + vote);
					return;
				}
