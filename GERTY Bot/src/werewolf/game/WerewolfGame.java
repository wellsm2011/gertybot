...
public class WerewolfGame implements ThreadManager
{
	...
	private VoteManager			votes					= new VoteManager(this.players);
	...
	public WerewolfGame(ForumThread init) throws IOException
	{
		...
		WerewolfGame.LOGGER.info("Game initalized: " + this.thread);
	}
	...
	private void addToGame(Command cmd)
	{
		...
		{
			WerewolfGame.LOGGER.info("Error parsing add command: " + ex.getMessage());
	...
	private void changeLynchType(Command cmd)
	{
		...
		String[] params = cmd.getParams().split(" ", 2); // Syntax: <type>[
															// <options>]
		if (params[0].matches("timed"))
		{
			...
			this.lynchType = 1; // Set lynch type to timed.
		} else if (params[0].matches("single"))
		{
			...
			this.lynchType = 2; // Set lynch type to single vote.
	...
	private void endGame(Command cmd)
	{
		try
		{
			...
			if (winners.size() > 0)
			{
				...
				try
				{
					this.thread.getContext().RECORD.addGame(this.thread, hosts, winners, losers);
				} catch (NullPointerException e)
				{
					WerewolfGame.LOGGER.log(Level.INFO, "Invalid context to record results: " + this.thread.getContext().toString() + ".", e);
				}
				...
				{
					WerewolfGame.LOGGER.log(Level.INFO, "Invalid context to complete signups: " + this.thread.getContext().toString() + ".", e);
				}
			}
		} catch (IOException | NullPointerException e)
		{
			WerewolfGame.LOGGER.log(Level.WARNING, "Unable to complete end command in " + this.thread.toString() + ".", e);
		}
	}
	...
	private void injurePlayer(Command cmd)
	{
		...
		else
		{
			...
			if (params.length > 1 && !params[1].isEmpty())
				...
				{
					WerewolfGame.LOGGER.info("Error parsing injure command: " + ex.getMessage());
	...
	{
		return user != null && (user.equals(this.host) || user.equals(this.cohost) || WerewolfGame.isMe(user));
	}
	...
	private void killPlayer(Command cmd)
	{
		...
		String[] params = cmd.getParams().split(", ?", 2); // Syntax: <target>[,
															// <message>]
		Player target = this.getPlayer(params[0]);
		if (target != null)
		{
	...
	private void makeVote(Command cmd)
	{
		...
		try
		{
			...
			User voted = this.getPlayer(cmd.getParams().replaceAll("^ *\\- *", ""));
	...
	private void parseCommands(ForumPost post) throws IOException
	{
		...
		for (Command command : post.getCommands())
		{
			...
			if (cmd.matches("^(join|in)$")) // Adds the user to the game.
				...
				{
					WerewolfGame.LOGGER.log(Level.WARNING, ex.getMessage(), ex);
				}
			else
	...
	private void removeFromGame(Command cmd)
	{
		...
		this.stateChange = this.players.remove(this.getPlayer(cmd.getParams()));
	}
	...
	public void reset() throws IOException
	{
		this.thread.reset();
		this.players.clear();
		this.votes = new VoteManager(this.players);
	...
	{
		Player target = this.getPlayer(cmd.getParams());
	...
	private void setCohost(Command cmd)
	{
		...
		String[] params = cmd.getParams().split(", ?", 2); // Syntax: <name>[,
															// <user ID>]
		...
		{
			WerewolfGame.LOGGER.info("Error parsing cohost command: " + ex.getMessage());
		}
		if (newCohost != null)
	...
	private void setStoryPost(Command cmd)
	{
		if (!this.isHost(cmd.getUser()))
			...
			this.storyPosts += "[*][url=" + cmd.getPost().getUrl() + "]" + cmd.getParams() + "[/url]\n";
	}
	...
	public boolean update() throws IOException
	{
		...
		WerewolfGame.LOGGER.info("Game parsed: " + this.thread.getTitle());
		...
		{
			WerewolfGame.LOGGER.warning("Cannot delete post in " + this.thread.getTitle() + ": " + ex.getMessage());
			ex.printStackTrace();
		}
